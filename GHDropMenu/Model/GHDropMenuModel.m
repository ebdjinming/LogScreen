...
#pragma mark - 数据筛选数据构造
- (NSArray *)creaFilterDropMenuData {
    /** 构造右侧弹出筛选菜单第一行数据 */
    /** 获取Log按钮的初始状态 */
    NSMutableArray *dataArr0 = [NSMutableArray array];
    GHDropMenuModel *dropMenuModel0 = [[GHDropMenuModel alloc] init];
    dropMenuModel0.logSwitch = YES;
    [dataArr0 addObject:dropMenuModel0];
    
    /** 构造右侧弹出筛选菜单第二行数据 */
    /** 设置时间！！！（什么数据都不用放置？？？） */
    NSMutableArray *dataArr1 = [NSMutableArray array];
    GHDropMenuModel *dropMenuModel1 = [[GHDropMenuModel alloc] init];
    [dataArr1 addObject:dropMenuModel1];

    /** 构造右侧弹出筛选菜单第三行数据 */
    /** 设置日志等级 */
    NSArray *row3 = @[@"verbose",@"debug",@"info",@"warn",@"error"];
...
//        dropMenuModel.tagSeleted = NO;
        [dataArr2 addObject:dropMenuModel];
    }
    
    /** 构造右侧弹出筛选菜单第四行数据 */
    /** 设置功能模块 */
    NSArray *row4 = @[@"1Layer",@"2Layer",@"3Layer",@"4Layer",@"5Layer",@"6Layer",@"7Layer"];
    NSMutableArray *dataArr3 = [NSMutableArray array];
    for (NSInteger index = 0 ; index < row4.count; index++) {
        GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
        dropMenuModel.tagName = [row4 by_ObjectAtIndex:index];
        [dataArr3 addObject:dropMenuModel];
    }
    
    /** 构造右侧弹出筛选菜单第五行数据 */
    /** 设置自定义输入框内容 */
    NSMutableArray *dataArr4 = [NSMutableArray array];
    GHDropMenuModel *dropMenuModel4 = [[GHDropMenuModel alloc] init];
    [dataArr4 addObject:dropMenuModel4];
    
    /** 构造右侧弹出筛选菜单第六行数据 */
    /** 获取搜索历史，并将搜索历史展现出来 */
    //Todo~~~~
    NSArray *row6 = @[@"注",@"册用",@"户登",@"录后",@"才能首页",@"发表",@"评论录"];
    NSMutableArray *dataArr5 = [NSMutableArray array];
    for (NSInteger index = 0 ; index < row6.count; index++) {
        GHDropMenuModel *dropMenuModel = [[GHDropMenuModel alloc]init];
        dropMenuModel.tagName = [row6 by_ObjectAtIndex:index];
        [dataArr5 addObject:dropMenuModel];
    }
    
    /** 设置构造右侧弹出筛选菜单每行的标题 */
    NSArray *sectionHeaderTitles = @[@"Log开关",@"时间选择",@"日志等级",@"功能模块",@"自定义筛选",@"搜索历史"];
    NSMutableArray *sections = [NSMutableArray array];
    ...
    for (NSInteger index = 0; index < sectionHeaderTitles.count; index++) {
        ...
        dropMenuModel.sectionHeaderTitle = sectionHeaderTitles[index];
        if (index == 0) {
            dropMenuModel.dataArray = dataArr0;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeSwitch;
            
        } else if (index == 1) {
            dropMenuModel.dataArray = dataArr1;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeTimeChose;
            
        } else if (index == 2) {
            dropMenuModel.dataArray = dataArr2;
            /** 多选 */
            dropMenuModel.isMultiple = YES;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeTag;
            
        } else if (index == 3) {
            dropMenuModel.dataArray = dataArr3;
            /** 多选 */
            dropMenuModel.isMultiple = YES;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeTag;
            
        } else if (index == 4) {
            dropMenuModel.dataArray = dataArr4;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeSingleInput;
            
        } else if (index == 5) {
            dropMenuModel.dataArray = dataArr5;
            /** 单选 */
            dropMenuModel.isMultiple = NO;
            dropMenuModel.sectionHeaderDetails = @"删除";
            dropMenuModel.sectionSeleted = YES;
            dropMenuModel.isLogHistory = YES;
            dropMenuModel.filterCellType = GHDropMenuFilterCellTypeTag;
        }
        [sections addObject:dropMenuModel];
    }
